...
#print(lammax)
#print(exctrans)
with open('top5_exc_data.txt','w') as ted:
    ted.write(' Transition  Lambda Max    Energy            Orbital\n')
    ted.write('   State        (nm)        (eV)           Transitions\n')
    for z in exctrans:
        ted.write('    ' + transstate[exctrans.index(z)] + '         ' + lammax[exctrans.index(z)] + '      ' + nrg[exctrans.index(z)] + '     ')
        for ax in z:
            if ax == z[0]:
                ted.write(ax + '\n')
            else:
